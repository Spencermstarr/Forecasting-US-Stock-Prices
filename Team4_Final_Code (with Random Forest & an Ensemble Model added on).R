...
## Forecasting Model #5: average neural network
set.seed(100)
...
# calculate the area under the above ROC curve
auc5 <- auc(nnetROC)
cat('Area under the ROC curve for our Average Neural Net:', round(auc5, 4))
...
# calculate the area under the above ROC curve
auc6 <- auc(marsROC)
cat('Area under the ROC curve for our MARS model: ', round(auc6, 4))
...
## Forecasting Model #8: K-Nearest Neighbors 
set.seed(100)
...
# calculate the area under the above ROC curve
auc8 <- auc(knnROC)
cat('Area under the ROC curve for our KNN model: ', 
    round(auc8, 4))




## Forecasting Model #9: Random Forest
set.seed(100)  # use the same seed for every model
...
auc9 <- auc(ROC_RF)
cat('Area under the ROC curve for our Random Forest model: ', round(auc9, 4))
...
## Forecasting Model #10: (Manually) Create an Ensemble Model
...
meta_model <- train(ensemble_data, class2015,
                    method = "glm",
                    metric = "ROC",
...
Ensemble_prob_2015 <- predict(meta_model, newdata = ensemble_data_2015, type = "prob")[, "Increase"]

# create a confusion matrix
ensemble_CFM <- confusionMatrix(data = meta_predictions_2015, reference = class2015, 
                                positive = "Increase")
ensemble_CFM
...
plot(ROC_ensemble, col = "red", lwd = 3, main = "ROC curve for our Ensemble Learning Model")

auc10 <- auc(ROC_ensemble)
cat('Area under the ROC curve for our Ensemble Learning Model: ', round(auc10, 4))

# variable importance evaluation
## NOTE: I did this the wrong way, but the result of it is 
## kind of interesting
imp_ensemble <- varImp(meta_model)
plot(imp_ensemble, top = 10, main = 'Variable Importance Plot for our Ensemble Learning Model')
...
# print out the Confusion Matrices for all 8 of our classification models again,
# but this time, print them out right on top of each other, one after the other
# in order to facilitate easy visual comparisons of each of their 
...
nnet_CFM           # the confusion matrix for our Neural Network model
